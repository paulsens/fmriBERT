    ...
    for i in range(0, len(ref_samples)):
        ...
        for copy in range(0, num_copies):
            ...
            while partner==i: #find a new partner with a different genre
                partner = random.choice(genre_sample_dict[rh_genre]) #all other reference indices of this genre
        ...
        if(i==0 and verbose):
            print("after i==0, training_samples has "+str(len(training_samples))+" samples, which should be "+str(2*seq_len)+ " and each sample has length "+str(len(training_samples[0]))+ " which each have length" +str(len(training_samples[0][0]))+".\n")
