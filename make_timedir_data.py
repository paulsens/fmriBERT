...
def make_timedir_data(dataset, threshold, hemisphere,  allowed_genres, seq_len=10, num_copies=1, include_test=1, test_copies=1, val_copies=1, standardize=1, detrend="linear", within_subjects=1, binary="timedir", seed=3, val_flag=0, verbose=1):
 ...
 # for testing that needs reproducibility
    ...
    voxel_dim = COOL_DIVIDEND #defined in Constants.py
    ...
    if dataset=="genre":
        ...
        for holdout in holdout_range:
            ...
            for i in range(0, len(ref_samples)):
                ...
                if(binary=="timedir"): # time direction task, possibly called temporal orientation instead?
                    ...
                    for j in range(start_idx, end_idx, incr):
                        this_input.append(coppy.deepcopy(ref_samples[i][j]))
            ...
            this_dir = timedir_datasets_path+str(time)+"-"+str(seq_len)+"TR_genre_HOruns/"
    ...
    # holding out data for audimg is more complicated than just holding out runs, so i have this separate
    elif dataset=="audimg":
        print("Use makepcdata.py for an audimg timedir dataset instead of this file.")
        quit(0)
...
if __name__=="__main__":
    ...
    # seed is 3 for pretrain, 4 for genre decode
    make_timedir_data(dataset="genre", threshold="23", hemisphere="left", seq_len=10, num_copies=1, standardize=1, detrend="linear", within_subjects=1, allowed_genres=range(0,10), val_flag=1, seed=4)