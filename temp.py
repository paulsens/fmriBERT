...
if __name__ == "__main__":
    with torch.autograd.set_detect_anomaly(True):
        ...
        if "-iteration" in opts:
            # iteration of the outer loop in the bash script, which isn't necessarily the same as the held out run
        ...
        # grab training data according to seq_len
        if seq_len==5:
            dataset="2022-06-12"
        elif seq_len==10:
            ...
            seq_len=None
            print("illegal sequence length, should be 5 or 10, quitting...")
            quit(0)
        ...
        if "-pretrain_task" in opts:
            ...
            print("Needs a pretraining task even if it's fresh, quitting...")
            quit(0)
        ...
        if "-finetune_task" in opts:
            ...
            # the 0 at the end is a counter for how many times ive saved a model with that name, so change that if you want a different one
            pretrained_model_states = "/isi/music/auditoryimagery2/seanthesis/opengenre/final/" + pretrain_task + "/states_" + str(heldout_run) + "0.pt"
            ...
            if finetune_task == "samegenre":
                ...
                dataset = "/isi/music/auditoryimagery2/seanthesis/pitchclass/finetuning/sametimbre/datasets/5/"
        else:
        ...
        hp_dict={
            ...
            "task":pretrain_task, # throughout the code, task is what the model was pretrained on
            # CLS and MSK task are also referring to what the model was trained on
            "CLS_task":"nextseq", #same_genre or nextseq
        ...
        for e in range(1, hp_dict["EPOCHS"]+1):
            ...
            for X_batch, y_batch in train_loader:
                ...
                for x in range(0,hp_dict["BATCH_SIZE"]):
                    ...
                    apply_masks(X_batch[x], y_batch[x], ref_samples, hp_dict, mask_variation, ytrue_multi_batch, sample_dists, ytrue_dist_multi1, ytrue_dist_multi2, batch_mask_indices, sample_mask_indices, mask_task=hp_dict["MSK_task"], log=log, heldout=False)
                ...
                if hp_dict["task"] == "CLS_only":
                    ...
                    loss = (loss_bin*CLS_task_weight) + (loss_multi*MSK_task_weight) #as per devlin et al, loss is the average of the two tasks' losses, in which case both weights would be 0.5
                    ...
                    for X_batch_val, y_batch_val in val_loader:
                        ...
                        if hp_dict["task"] == "binaryonly":
                            ...
                            loss = (loss_bin_val * CLS_task_weight) + (
                                        loss_multi_val * MSK_task_weight)  # as per devlin et al, loss is the average of the two tasks' losses, in which case both weights would be 0.5
        ...
        #         print(name, param.data)
        modelcount=0
        model_path = "/isi/music/auditoryimagery2/seanthesis/thesis/pairedfinetune/"+str(hp_dict["finetune_task"])+"_states_"+str(hp_dict["heldout_run"])+str(modelcount)+".pt"
        while(os.path.exists(model_path)):
            modelcount+=1
            model_path = "/isi/music/auditoryimagery2/seanthesis/thesis/pairedfinetune/"+str(hp_dict["finetune_task"])+"_states_"+str(hp_dict["heldout_run"])+str(modelcount)+".pt"

        torch.save(model.state_dict(),model_path)
        model_path = "/isi/music/auditoryimagery2/seanthesis/thesis/pairedfinetune/"+str(hp_dict["finetune_task"])+"_full_" + str(hp_dict["heldout_run"]) + str(
            modelcount) + ".pt"
