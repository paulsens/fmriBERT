...
if __name__ == "__main__":
    ...
    # For the gaussian toy data
    samples_fp = open("/Users/sean/Desktop/current_research/fmriBERTfix/fmriBERT/pitchclass/samples/gaussian_samples.p",
                      "rb")
    labels_fp = open("/Users/sean/Desktop/current_research/fmriBERTfix/fmriBERT/pitchclass/samples/gaussian_labels.p",
                     "rb")
    NS_X = torch.FloatTensor(pickle.load(samples_fp))
    NS_y = torch.FloatTensor(pickle.load(labels_fp))
    samples_fp.close()
    labels_fp.close()
    train_data = TrainData(NS_X, NS_y) #make the TrainData object
    train_loader = DataLoader(dataset=train_data, batch_size=BATCH_SIZE, shuffle=True) #make the DataLoader object
    ...
    torch.save(model, "/Users/sean/Desktop/current_research/fmriBERTfix/fmriBERT/pitchclass/experiment1.pt")
