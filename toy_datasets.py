...
def make_toy_data2(threshold, hemisphere, seq_len=5, test_copies=1, standardize=1, binary="same_genre", multiclass="genre_decode", verbose=1):
    #runs_dict is defined in Constants.py
    test_runs = runs_dict["Test"]
    training_runs = runs_dict["Training"]
    ...
    for sub in ["001", "002", "003", "004", "005"]:
        ...
        for t in range(0, timesteps//seq_len):
            ...
            this_label = [same_genre_choice, this_genre]
            #append to aggregate lists
            if(this_genre==1 or this_genre==6):
                training_samples.append(this_sample)
                training_labels.append(this_label)
                training_samples.append(copy.deepcopy(this_sample))
                training_labels.append(copy.deepcopy(this_label))
                #increase count
                count = count+2
    ...
    # the aggregate lists are complete, let's get the second half of each sample
    for i in range(0, len(training_samples), 2):
        same_genre = training_labels[i][0]
        this_genre = training_labels[i][1]


        potentials_same = genre_sample_dict[this_genre] #a list of indices in training_samples with this genre
        choice = this_genre
        if(choice==1):
            choice = 6
        else:
        ...
        potentials_diff = genre_sample_dict[choice] #a list of indices in training_samples with a different genre

        partner = i
        while(partner==i): #get a partner different from self
            partner_choice=randint(0,len(potentials_same)-1) #pick something in the list of potentials
            partner=potentials_same[partner_choice] #get the index in training_samples corresponding to that choice
        ...
        # go again for a partner of different genre
        partner = i
        while(partner==i): #get a partner different from self
            partner_choice=randint(0,len(potentials_diff)-1) #pick something in the list of potentials
            partner=potentials_diff[partner_choice] #get the index in training_samples corresponding to that choice

        for j in range(0,seq_len):
            #training_samples[i] is the current sample
            #training_samples[partner] is the randomly chosen sample with same/different genre
            training_samples[i+1].append(training_samples[partner][j])
        training_labels[i+1].append(training_labels[partner][1]) #get the genre index of the partner
...
# NSP instead of same-genre binary task (not implemented)
def make_toy_data3(threshold, hemisphere, seq_len=5, test_copies=1, standardize=1, binary="same_genre", multiclass="genre_decode", verbose=1):
    #runs_dict is defined in Constants.py
    test_runs = runs_dict["Test"]
    training_runs = runs_dict["Training"]
