...
if __name__=="__main__":
    ...
    task_idx = opts.index("-pretrain_task")
    
    pretrain_task = args[task_idx]
    ...
    if env=="local":
        ...
        data_path = "/Volumes/External/opengenre/preproc/training_data/2022-11-14/"
    if env=="discovery":
        pretrained_model_states = "/isi/music/auditoryimagery2/seanthesis/opengenre/final/"+pretrain_task+"/states_"+pretrain_idx+"0.pt"
        data_path = "/isi/music/auditoryimagery2/seanthesis/opengenre/preproc/training_data/2022-11-14/"

    # load the training and validation data
    with open(data_path+"left_samples"+str(thiscount)+".p", "rb") as samples_fp:
        train_X = pickle.load(samples_fp)
    with open(data_path+"left_labels"+str(thiscount)+".p", "rb") as labels_fp:
        train_y_detailed = pickle.load(labels_fp)
    with open(data_path+"left_valsamples"+str(thiscount)+".p", "rb") as valsamples_fp:
        val_X = pickle.load(valsamples_fp)
    with open(data_path+"left_vallabels"+str(thiscount)+".p", "rb") as vallabels_fp:
